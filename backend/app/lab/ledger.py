...
_INTEGRITY_DIGEST_RE = re.compile(r"(?:sha256:)?[0-9a-f]{64}")
...
async def append_event(
    ...
    *,
    ...
    # Top-level digests are generated by the control plane and bind replay.
    # Runtime/player content remains nested under ``payload`` and stays redacted.
    if isinstance(payload, dict):
        for key, value in (envelope.payload or {}).items():
            if (
                key.endswith("_digest")
                and isinstance(value, str)
                and _INTEGRITY_DIGEST_RE.fullmatch(value)
            ):
                payload[key] = value
