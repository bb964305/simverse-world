...
def _required_database() -> tuple[str, str]:
    """锁序 PG 证据的环境守卫：**没要求就 skip，要求了却给不出才 fail**。

    原实现把「未设 `LAB_POSTGRES_REQUIRED`」也判成 `pytest.fail`，于是本用例在
    任何没有 PG 的机器上都是 `ERROR` 而不是 skip。范式取自
    ...
    if os.environ.get("LAB_POSTGRES_REQUIRED", "").lower() not in TRUE:
        pytest.skip("LAB_POSTGRES_REQUIRED is not set — opt-in lock-order evidence")
    ...
    if missing:
        pytest.fail("LAB_POSTGRES_REQUIRED=1 but lock-order environment is "
