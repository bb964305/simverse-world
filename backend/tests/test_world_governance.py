"""P3 world governance: enabled lore flow, disabled kinds, and treasury fuel."""
...
# ── disabled mutation kinds ───────────────────────────────────────────

@pytest.mark.anyio
async def test_add_location_is_rejected_before_persistence(gov_env):
    factory = gov_env
    patch = {
        "slug": "observatory",
        "data": {"name": "天文台", "bounds": [5, 88, 15, 96]},
    }
    async with factory() as db:
        with pytest.raises(psvc.ProposalError, match="not enabled"):
            await psvc.create_proposal(
                db,
                kind="add_location",
                title="加建天文台",
                rationale="研究员发现了适合观星的空地",
                patch=patch,
                author_slug="sage",
                cost_sc=0,
            )
    ...
    async with factory() as db:
        count = await db.scalar(
            select(func.count()).select_from(WorldChangeProposal)
        )
    assert count == 0
