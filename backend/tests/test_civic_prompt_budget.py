...
#: 拍板预算里镇务那条道的**开闸建议值**,不是随手写的上界:2 = 一次选举闭环里
#: 非赢家能看到的结果档条数。
#:
#: world_event 那条道的建议值(1)不在这里另立常量 —— 下面那条用例把它
#: ``parametrize`` 成 ``[0, 1]``,0 是对照、1 就是建议值。写死一个只被读一次的
#: 同义常量,反而会与 parametrize 里的字面量无声漂开。
#: (口径:实质事件约 1.6 条/人/周,一个坑约等于「记得最近那件事」;2 个坑会让两周
#: 前的节庆压住这周的。)
