...
const COPY = {
  'zh-CN': {
    ...
    browser: '官网页面', api: '游戏 API', runtime: 'Agent 挂机循环', chain: 'Robinhood Chain', contract: 'Agent Registry', online: '已连接', checking: '检测中…', retry: '重新检测', notChecked: '等待检测',
    ...
    steps: [
      ...
      ['03', '创建你的游戏居民', '首次登录进入三步入城流程。填写公开名称、选择形象后，系统创建可游玩的居民身份。'],
      ['04', '注册链上 Agent 身份', '继续在注册页确认交易。系统会铸造一个不可转让的 Agent Passport；链上确认后才完成入城，Passport 永久归当前钱包所有。'],
    ...
    storage: [
      ...
      ['Agent 运行时', 'Agent Passport 证明链上身份与版本；世界模拟持续运行。个人托管模型只有在配置推理服务后才会自主挂机。'],
    ],
    ...
    openStudio: '登录后打开 Agent Studio', economy: '查看 SIM 经济模型', explorer: '查看主网合约', social: '官方社区', checked: '检测时间',
  },
  en: {
    ...
    browser: 'Website', api: 'Game API', runtime: 'Agent world loop', chain: 'Robinhood Chain', contract: 'Agent Registry', online: 'Online', checking: 'Checking…', retry: 'Check again', notChecked: 'Not checked',
    ...
    steps: [
      ...
      ['03', 'Create your game resident', 'The three-step city registration asks for a public name and visual origin, then creates a playable resident.'],
      ['04', 'Register an onchain Agent identity', 'Continue in registration and confirm the transaction. Entry finishes after a non-transferable Agent Passport is minted to the connected wallet.'],
    ...
    storage: [
      ...
      ['Agent runtime', 'The Agent Passport proves identity and versions onchain while the world simulation keeps running. Personal hosted-model autonomy requires an inference provider.'],
    ],
    ...
    openStudio: 'Open Agent Studio after sign-in', economy: 'Read the SIM economy', explorer: 'View mainnet contract', social: 'Official community', checked: 'Checked',
  },
} as const
  ...
  return (
    <div className="web3-guide">
      <header className="guide-header">
        <Link className="guide-brand" to="/"><BrandLogo size={42} eager /> SIMVERSE</Link>
      ...
      <main>
        ...
        <section className="guide-live" aria-labelledby="live-check-title">
          ...
          <div className="guide-live__panel" aria-live="polite">
            ...
            <StatusCard label={copy.runtime} item={report?.runtime} checking={checking} />
        ...
        <section className="guide-cta"><h2>{copy.connect}</h2><div className="guide-actions"><Link className="guide-button guide-button--primary" to="/login">{copy.connect}</Link><Link className="guide-button" to="/web3">{copy.openStudio}</Link><Link className="guide-button" to="/economy">{copy.economy}</Link></div><p>{copy.social}</p><BrandSocialLinks className="brand-socials--guide-cta" /></section>
      </main>
    </div>
  )
}
