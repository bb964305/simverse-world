...
export function ExplorationCodex() {
  ...
  if (!data) return <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>加载中…</div>
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      <h1 style={{ fontSize: 20, fontWeight: 700, marginBottom: 4, color: 'var(--text-primary)' }}>探索图鉴</h1>
      <div style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 16 }}>
        已探索 <span style={{ color: 'var(--accent-green)', fontWeight: 700 }}>{data.visited}</span> / {data.total} 个地点
        {secretsTotal > 0 && (
          <span style={{ marginLeft: 12 }}>⭐ 彩蛋 {secretsFound} / {secretsTotal}</span>
        )}
      </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>
              {current.visited ? '📖' : '🌫️'} {current.visited ? current.name : '？？？'}
            </span>
            {current.visited && (
              <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>到访 {current.visit_count} 次</span>
            )}
            {current.secret_found && <span style={{ fontSize: 11 }}>⭐ 已发现彩蛋</span>}
            {current.has_secret && !current.secret_found && current.visited && (
              <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>这里似乎藏着什么…</span>
            )}
          </div>
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.7, marginTop: 8 }}>
            {current.visited
              ? (current.lore ?? '这个地方还没有记载。')
              : '尚未探索 — 亲自去看看吧。'}
          </div>
        </div>
      )}
        ...
        {data.locations.map((l) => (
          <button key={l.location_id} onClick={() => setSelected(l.location_id)} style={{
            ...
            <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>
              {l.visited ? `到访 ${l.visit_count} 次` : '未探索'}
            </div>
          </button>
        ))}
      </div>
