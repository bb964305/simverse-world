    ...
    >
      ...
      >
        <div className="game-dialog-header" style={{
          ...
          <div>
            <div id="labterminal-dialog-title" style={{ fontWeight: 800, fontSize: 15, color: ACCENT }}>📊 {isEn ? 'Lab Terminal' : '实验楼终端'}</div>
            <div style={{ ...muted, marginTop: 2 }}>{isEn ? 'Read-only runtime status · no publish, settlement, or approval actions' : '只读运行状态 · 无发布 / 放款 / 审批操作'}</div>
          </div>
          <button onClick={() => setOpen(false)} className="game-dialog-close" aria-label={isEn ? 'Close Lab Terminal' : '关闭实验楼终端'}>✕</button>
        </div>
        ...
        <div className="game-lab-split" style={{ display: 'flex', gap: 12, padding: 20 }}>
          <div style={{ width: 200, borderRight: '1px solid var(--border)', paddingRight: 12 }}>
            <div style={{ ...muted, marginBottom: 8 }}>{isEn ? 'My tasks' : '我的委托'}</div>
            {loading && <div style={muted}>{isEn ? 'Loading…' : '加载中…'}</div>}
            {!loading && tasks.length === 0 && <div style={muted}>{isEn ? 'No tasks yet' : '还没有委托'}</div>}
            {tasks.map((t) => {
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {!selected && <div style={muted}>{isEn ? 'Choose a task to inspect its runtime' : '选择一个委托查看运行状态'}</div>}
            {selected && !run && <div style={muted}>{isEn ? 'This task has not started.' : '该委托尚未开始运行。'}</div>}
            {selected && run && (() => {
              const d = resolveLabDisplay({ runStatus: run.status }, locale)
              return (
                <div>
                  <div style={{ ...card, display: 'flex', gap: 12, alignItems: 'center' }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: ACCENT }}>
                      {isEn ? 'Runtime' : '运行状态'}: {d.run ? d.run.label : '—'}
                    </span>
                    <span style={muted}>
                      {isEn ? 'Adapter' : '适配器'} {run.adapter}{formatLabRunProfile(run, locale) ? ` · ${formatLabRunProfile(run, locale)}` : ''}
                    </span>
                    {run.error && <span style={{ fontSize: 12, color: '#ef4444' }}>{isEn ? 'Error' : '错误'}: {run.error}</span>}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 300, overflowY: 'auto' }}>
                    ...
                    {steps.length === 0 && <div style={muted}>{isEn ? 'No runtime steps' : '暂无步骤记录'}</div>}
                  </div>
                </div>
              )
            })()}
