...
| **G4' env 一致性闸** | ✅ 已修 | 不变式 1 改为认 sidecar 键（精确白名单，非前缀）。第一版按前缀写被同批新增的守卫当场抓出**盖住 33 个真 Settings 字段** |
| **F2 PG 证据守卫** | ✅ 已修 | 三个文件的守卫语义是反的（运维**没要**证据时反而 `pytest.fail`）。改为「没要求就 skip，要求了却给不出才 fail」；release gate 由 `conftest.py:143-147` 把任何 skip 判失败兜底 |
| **F1 默认门 marker 选择** | ✅ 已修 | `addopts` 只排了 `lab_oci`，另外四个 marker 从没同步过。新增 `tests/test_pytest_default_selection.py` 三条静态断言把口径分叉变成开工即红 |

**门禁变化（全量，双向差集验证新增失败 = 0）**

| | failed | passed | skipped | errors |
|---|---:|---:|---:|---:|
| 合并基线 `6128ecb` | 50 | 2558 | 25 | 17 |
| 本轮之后 | **49** | **2572** | **1** | **0** |

`17 errors` 归零意味着 **CI 可以重新变成一道真门**——error 是 `xfail` 罩不住的，只要它在，`quarantine` 之类的隔离机制就无从谈起。剩下的 49 条全部落在 `test_lab_*` 下（F6 的清零口径，须与 `feat/lab-codex-runtime` 协调）。
