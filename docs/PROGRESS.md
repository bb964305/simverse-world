...
- [ ] P1-1 LLM 计量（llm_usage 表）+ 预算熔断 + 分级模型 ← 🔥 功能的闸门　**（进行中，2/8 子提交已落）**
  ...
  - **剩余 子提交 3–8（下个会话按序做）**：
    3. 熔断三级（80% 背景降频 / 95% 背景规则化 / 100% 只保玩家可见）+ per-user 日预算（映射灵魂币）+ forge 单请求上限；每级规则兜底不白屏；路由：背景锁 haiku、玩家可见可配置（E-24/E-18）。**新表已就绪**：熔断器按窗口 `SUM(cost_usd)` from `llm_usage` 即可，无需再改计量。
