...
| **2. 社会地基** | 进行中 | S2-1、S1-3、S1-5、S2-5 已完成且已在 vm212 开闸；S1-1 声誉已实现，F1 语义修复已合入但未开闸 | 官职、舆论、财政、政策和声誉形成可观察闭环——现在卡的不再是「信号只有负向」（F1 已修），而是 `rep_credit_min_score` 未用真实分布重标定，F1/F2/F3 的接线与配置面已于 2026-08-05 收口（近期优先级 5），剩 `rep_credit_min_score` 重标定 |
