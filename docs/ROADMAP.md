...
| **2. 社会地基** | 进行中 | S2-1、S1-3、S1-5、S2-5 已完成且已在 vm212 开闸；S1-1 声誉已实现，F1 语义修复已合入但未开闸 | 官职、舆论、财政、政策和声誉形成可观察闭环——现在卡的不再是「信号只有负向」（F1 已修），而是 `rep_credit_min_score` 未用真实分布重标定，以及 F1/F2/F3 三条线**都还没接进 `nightly_cron`**（见下） |
...
| **5. 政治深化** | 已开局（F2/F3 合入主线但未接线） | 抽签任官、轮值议事、集会投票、卸任审计、陪审、丑闻与政体演变 | 权力流转和冲突能形成持续故事。**已落代码**：F3 的任期到期触发补选（补掉「无限期无镇长」断链）+ 卸任财政审计；F2 的公民权晋升/撤销三档模型。**尚未生效**：两者都没挂进 `nightly_cron`，开关也全默认关 |
...
7. Lab 继续保持 Mock-only；`feat/lab-codex-runtime` 合入前必须把它的 `051/052/053` 重编号——它的 `051_add_lab_codex_model_tier` 与主线的 `051_add_civic_standing_history` **都挂在 `050` 上，合并即双头**。重编号完成后：将已在 ARM staging 健康运行的 Codex Runtime 接入主站 staging，部署 Lab 迁移和玩家报价 UI，跑通“发布 code 委托 → 分档调度 → 直播 → 验收 → 安全下载”；在逐任务强隔离、外部制品存储/扫描和 attestation 完成前不进入生产灰度。
