...
| 生产修缮 | heat 时区混比、预算静默告警和 SBTI backfill 工具已完成并部署；26/26 居民已有完整 A2 维度 | 等下一轮 civic poll 复核回填后的投票分布 |
| 社会地基 | S2-1 职位实体化和 S1-3 舆论动力学已完成、迁移到 047，并在 vm212 开闸 | 实现 S1-5 财政、S2-5 政策、S1-1 声誉 |
...
| **1. 上线收尾** | 基本完成 | vm212 部署、迁移 047、heat 修复、告警、SBTI 回填、TownHall/LabTerminal | 下一轮投票分布复核完成，真实成本持续在预算内 |
| **2. 社会地基** | 进行中 | 已完成 S2-1、S1-3；待做 S1-5、S2-5、S1-1 | 官职、舆论、财政、政策和声誉形成可观察闭环 |
...
1. 等下一轮 civic poll，核对 SBTI 回填是否消除 option-0 偏差；同时持续核对 `llm_usage` 与真实账单。
2. 并行实现 S1-5 财政闭环与 S2-5 政策分级审批，保持各自 feature gate 默认关闭。
3. 在前两项接口稳定后实现 S1-1 声誉，并让选举、关系和市政厅消费同一份声誉数据。
4. 明确玩家角色是否参与 NPC 自治，区分“注册人口”和“自治居民”，再决定 25-40 人扩容策略。
5. 补夜间任务错过窗口的补跑机制、聊天锁 TTL、多实例状态与告警可观测性。
